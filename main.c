...
static char *Assembler = "riscv64-linux-gnu-as";
...
static void parseArgs(int Argc, char **Argv) {
    ...
    for (int I = 1; I < Argc; I++) {
        ...
        // 其他情况则匹配为输入文件, including "-", which is for stdin
...
// 调用汇编器(system toolchains, not rvcc)
static void assemble(char *Input, char *Output) {
    char *Cmd[] = {Assembler, "-c", Input, "-o", Output, NULL};
    runSubprocess(Cmd);
}
